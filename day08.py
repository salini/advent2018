...
class Node(object):
    ...
    def read(self, isequence):
        nb_children = isequence.next()
        nb_metadata = isequence.next()

        for nchild in range(nb_children):
            n = Node()
            n.read(isequence)
            self.children.append(n)

        for nmeta in range(nb_metadata):
            self.metadata.append(isequence.next())

    def get_sum_metadata(self):
...
def get_recursive_sum_metadata(node):
    sum_metadata = node.get_sum_metadata()
    for c in node.children:
        sum_metadata += get_recursive_sum_metadata(c)

    return sum_metadata

def get_sum_of_metadata_entries():
    sequence = _load_sequence()

    root = Node()
    root.read(iter(sequence))

    sum_metadata = get_recursive_sum_metadata(root)
    return sum_metadata
...
If a referenced child node does not exist, that reference is skipped. A child node can be referenced multiple time and counts each time it is referenced. A metadata entry of 0 does not refer to any child node.
...
def get_recursive_node_value(node):
    if node.is_leaf():
        return node.get_sum_metadata()
    else:
        values = [get_recursive_node_value(c) for c in node.children]

        nodeval = 0
        for ref in node.metadata: #ref starts at 1
            ref0 = ref-1
            if ref0 < len(values):
                nodeval += values[ref0]
